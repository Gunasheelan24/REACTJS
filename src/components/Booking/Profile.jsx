...
const Profile = () => {
  ...
  const { user, profileState, setprofileState, setuserName } =
    useContext(Store);
  ...
  async function handleSubmit(e) {
    try {
      ...
      let newPicture = await axios({
        method: "post",
        url: `https://parisairwaysversionone.onrender.com/ParisAirways/v1/newprofile/${user.email}`,
        data: formData,
      });
      setprofileState(
        `https://parisairwaysversionone.onrender.com/${newPicture.data.profile}`
      );
    } catch (error) {
      console.log(error);
    }
          ...
          <div className="container">
            <div className="row">
              ...
              <div className="col-sm-6 mt-5 d-flex flex-column">
                ...
                <button
                  onClick={logout}
                  className="btn fw-bold btn-danger mt-4"
                >
                  LOGOUT
                </button>
              </div>
