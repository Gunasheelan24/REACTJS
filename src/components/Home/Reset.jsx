...
const Reset = () => {
  ...
  let onSubmitHandler = async (value, { resetForm }) => {
    try {
      await axios({
        method: "post",
        url: `https://parisairwaysversionone.onrender.com/ParisAirways/v1/verfyToken/${email}`,
        data: {
          value,
        },
      });
  ...
  const validationSchema = yup.object({
    password: yup
      ...
      .min(5, "ResetPassword Must Have 5 Digits"),
    confirmPassword: yup
      .string()
              ...
              {({ errors, touched }) => (
                <Form className="mt-3">
                  ...
                  {errors.password && touched.password && (
                    <p className="text-white mt-1 password">
                      {errors.password}
                    </p>
                  )}
