...
const Login = () => {
  const { user, setuser, setuserName, setprofileState, profileState } =
    useContext(Store);
  ...
  const handleSubmit = async (value, { resetForm }) => {
    try {
      ...
      const data = await axios({
        ...
        url: "https://parisairwaysversionone.onrender.com/ParisAirways/v1/loginPage",
        withCredentials: true,
      });
      let cookie = new Cookies();
      setuserName(cookie.cookies);
      setuser(data.data.result);
      setprofileState(data.data.result.profile);
      console.log(data.data.result.profile);
      setTrue(true);
